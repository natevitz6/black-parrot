...
module bp_uce_hit_pre
  ...
  logic [$clog2(lookahead_depth+1)-1:0] remaining_prefetches_r, remaining_prefetches_n;

  logic [$clog2(miss_count+1)-1:0] count_r, count_n;
  ...
  logic [addr_width_p-1:0] expected_hit_addr_r, expected_hit_addr_n;
  ...
  always_comb begin
    ...
    expected_hit_addr_n     = expected_hit_addr_r;
    ...
    if (miss_v_r) begin
      if (miss_count == 1) begin
        ...
        prefetch_addr_n        = miss_addr_r + line_size_lp;
        expected_miss_addr_n   = miss_addr_r + line_size_lp;
        expected_hit_addr_n    = miss_addr_r + line_size_lp;
        prefetch_v_n           = 1'b1;
    ...
    // Continue stream if hit matches expected address
    if (hit_v_r) begin
      if (hit_addr_r == expected_hit_addr_r && stream_active_r) begin
        prefetch_v_n    = 1'b1;
        expected_hit_addr_n = expected_hit_addr_r + line_size_lp;
      end /*else begin
        //stream_active_n = 1'b0;
        //prefetch_v_n    = 1'b0;
      end */
  ...
  always_ff @(posedge clk_i) begin
    if (reset_i) begin
      ...
      expected_hit_addr_r    <= expected_hit_addr_n;
