...
module bp_uce_multi_pre
  ...
  logic [streams_p-1:0] miss_match_lo, hit_match_lo, invalid_stream_lo;

  logic [$clog2(streams_p)-1:0] match_idx, alloc_idx, issue_idx;
  logic match_found, alloc_found, issue_found;

  logic [$clog2(streams_p)-1:0] prefetch_stream_n, prefetch_stream_r, least_active_stream_n, least_active_stream_r;
  ...
  always_comb begin
    ...
    least_active_stream_n   = least_active_stream_r;
    ...
    for (int i = 0; i < streams_p; i++) begin
      ...
      invalid_stream_lo[i] = !stream_active_r[i] && (count_r[i] == '0);
    end

    for (int i = 0; i < streams_p; i++) begin
      // Train / start stream from the registered miss from the previous cycle
      if ((miss_match_lo[i] || hit_match_lo[i]) && !match_found) begin
        match_idx = i[$clog2(streams_p)-1:0];
        match_found = 1'b1;
      end
      if (invalid_stream_lo[i] && !alloc_found) begin
        alloc_idx = i[$clog2(streams_p)-1:0];
        alloc_found = 1'b1;
      end
    end
    ...
    if (miss_v_r) begin
      if (match_found) begin
        ...
        stream_active_n[least_active_stream_r] = 1'b0;
        remaining_prefetches_n[least_active_stream_r] = '0;
        expected_miss_addr_n[least_active_stream_r] = miss_addr_r + line_size_lp;
        count_n[least_active_stream_r] = 1'b1;
      end
    ...
    for (int i = 0; i < streams_p; i++) begin
      if (!issue_found && stream_active_r[i] && (remaining_prefetches_r[i] != '0)) begin
        issue_idx = i[$clog2(streams_p)-1:0];
        issue_found = 1'b1;
      end
    end
  ...
  always_ff @(posedge clk_i) begin
    if (reset_i) begin
      ...
      least_active_stream_r  <= least_active_stream_n;
...
`ifndef SYNTHESIS
      ...
      if (miss_v_r && (miss_addr_r == expected_miss_addr_r) && (count_r == miss_count) && !stream_active_r) begin
        $display("[PREF-STREAM-START] time=%0t miss_addr=%h prefetch_addr=%h rem=%0d",
                 $time, miss_addr_r, miss_addr_r + line_size_lp, lookahead_depth);
      end
      ...
      if (hit_v_r && (hit_addr_r == expected_hit_addr_r && stream_active_r)) begin
        $display("[PREF-HIT] time=%0t hit_addr=%h expected=%h active=%0b",
                 $time, hit_addr_r, expected_hit_addr_r, stream_active_r);
      end else if (hit_v_r) begin
        $display("[PREF-HIT-MISMATCH] time=%0t hit_addr=%h expected=%h active=%0b",
                 $time, hit_addr_r, expected_hit_addr_r, stream_active_r);
      end
`endif
